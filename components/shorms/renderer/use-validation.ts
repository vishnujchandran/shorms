...
export function useValidation(options: UseValidationOptions) {
  ...
  const pendingRef = useRef<
    Map<string, Promise<ValidationResult | true | string>>
  >(new Map())
  ...
  const getCachedResult = useCallback(
    (fieldId: string, value: any): ValidationResult | null => {
  ...
  const cacheResult = useCallback(
    (fieldId: string, value: any, result: ValidationResult, ttl: number) => {
  ...
  const normalizeResult = useCallback(
    (result: ValidationResult | true | string): ValidationResult => {
      if (result === true) {
        return { valid: true }
      }
  ...
  const runSyncValidation = useCallback(
    (field: FormField, value: any, allValues: FormValues): ValidationResult => {
      ...
      if (
        ...
        return {
          valid: false,
          message:
            field.validation?.errorMessage || `${field.label} is required`,
          severity: "error",
          blocking: true,
        }
      }
  ...
  const validateFieldDebounced = useCallback(
    (fieldId: string): Promise<ValidationResult> => {
  ...
  const validatePage = useCallback(
    async (pageId: string): Promise<Map<string, ValidationResult>> => {
  ...
  const validateCrossField = useCallback(async (): Promise<
    Map<string, ValidationResult>
  > => {
    const results = new Map<string, ValidationResult>()
